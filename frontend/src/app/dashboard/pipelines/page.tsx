...
export default function PipelinesPage() {
  ...
  return (
    <>
      <header className="flex justify-between items-center mb-xl border-b border-outline-variant/30 pb-md animate-fade-in">
        <div>
      ...
      <div className="flex flex-col gap-md animate-fade-in">
        {loading ? (
          ...
          pipelines.map((p, i) => (
            <div key={p.id || i} className="bg-[#111118] border border-outline-variant/30 rounded-lg p-lg hover-glow transition-all duration-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-md animate-slide-up" style={{ animationDelay: `${i * 100}ms` }}>
              <div className="flex flex-col gap-sm">
                <div className="flex items-center gap-md">
                  <span className="material-symbols-outlined text-primary text-[24px]">dataset</span>
                  <h3 className="font-headline-md text-headline-md text-white">{p.name || p.id}</h3>
                ...
                <div className="flex gap-lg font-code-md text-sm text-on-surface-variant">
                  <span className="flex items-center gap-xs"><span className="material-symbols-outlined text-[16px]">schedule</span> {p.updated || "Recently created"}</span>
                  <span className="flex items-center gap-xs"><span className="material-symbols-outlined text-[16px]">segment</span> {p.chunks || 0} chunks</span>
                  <span className="flex items-center gap-xs"><span className="material-symbols-outlined text-[16px]">forum</span> {p.queries || 0} queries</span>
                </div>
              </div>
              <div className="flex items-center gap-sm w-full md:w-auto mt-sm md:mt-0">
                <Link href={`/dashboard/pipelines/${p.id || p}`} className="flex-1 md:flex-none border border-outline-variant hover:border-primary hover:text-primary bg-transparent text-white font-label-caps text-xs px-md py-sm rounded transition-colors flex items-center justify-center gap-xs group">
                ...
                <button 
                  ...
                  className="flex-1 md:flex-none border border-outline-variant hover:border-primary hover:text-primary bg-transparent text-white font-label-caps text-xs px-md py-sm rounded transition-colors flex items-center justify-center gap-xs"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0e0d16]/80 backdrop-blur-md p-md animate-fade-in">
          <div className="bg-[#111118] border border-outline-variant/50 rounded-xl w-[90vw] md:w-[600px] max-w-full shadow-[0_0_40px_rgba(108,99,255,0.1)] flex flex-col animate-slide-up">
            <div className="flex justify-between items-center px-xl py-lg border-b border-outline-variant/30 bg-[#1b1b24] rounded-t-xl">
              ...
              <button onClick={() => !isCreating && setIsModalOpen(false)} className="text-on-surface-variant hover:text-error transition-colors flex items-center justify-center">
            ...
            <div className="px-xl py-lg flex flex-col gap-xl">
              <div className="flex flex-col gap-xs w-full relative">
                ...
                <input 
                  ...
                  value={pipelineName}
                  onChange={(e) => {
                    if (e.target.value.length <= 50) setPipelineName(e.target.value);
                  }}
                  className={`w-full bg-[#1f1f28] border ${nameError ? 'border-error focus:ring-error/50' : 'border-outline-variant focus:border-primary focus:ring-primary/50'} focus:ring-1 rounded px-md py-sm font-code-md text-sm text-white placeholder:text-zinc-600 outline-none transition-all`} 
              ...
              <div className="flex flex-col gap-xs w-full relative mt-sm">
                <label className="font-label-caps text-xs text-on-surface-variant uppercase">Upload Documents</label>
                <div 
                  className={`w-full border-2 border-dashed rounded-lg py-xl px-lg flex flex-col items-center justify-center gap-md transition-all cursor-pointer ${
                    fileError ? 'border-error bg-error/5' : 
                    isDragging ? 'border-primary bg-primary/10 scale-[1.02]' : 
                    'border-outline-variant bg-[#1f1f28]/50 hover:border-primary hover:bg-[#1f1f28]'
                  }`}
                  onDrop={handleDrop}
              ...
              {isCreating && (
                <div className="w-full mt-2 animate-fade-in">
                  ...
                  <div className="w-full h-1.5 bg-[#1b1b24] rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-primary transition-all duration-300 shadow-[0_0_10px_rgba(108,99,255,0.8)]"
            ...
            <div className="px-xl py-md border-t border-outline-variant/30 bg-[#1b1b24] rounded-b-xl flex justify-end gap-md">
              <button 
                ...
                disabled={isCreating}
                className="font-label-caps text-sm text-on-surface-variant hover:text-white px-md py-sm transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button 
