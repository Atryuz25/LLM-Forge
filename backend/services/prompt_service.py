...
async def ab_test(
    ...
    # You can make this smarter later with RAGAS
...
async def get_ab_test_history() -> list:
    try:
        from mlflow.tracking import MlflowClient
        client = MlflowClient()
        runs = client.search_runs(experiment_ids=["0"], max_results=20, order_by=["start_time DESC"])
        history = []
        for r in runs:
